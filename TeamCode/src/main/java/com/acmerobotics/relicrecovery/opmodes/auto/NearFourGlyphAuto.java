...
public class NearFourGlyphAuto extends AutoOpMode {
    ...
    protected void run() {
        ...
        Trajectory floorToPit = robot.drive.trajectoryBuilder(stoneToFloor.end())
                .turn(-Math.PI / 4)
        ...
        Trajectory pitToCrypto1 = robot.drive.trajectoryBuilder(floorToPit.end())
        ...
        if (robot.intake.getMode() == Intake.Mode.AUTO) {
            ...
            robot.drive.followTrajectory(robot.drive.trajectoryBuilder(cryptoToPit2.end())
                    .forward(12)
                    .back(12)
                    .turn(Math.PI / 4)
                    .build());
        ...
        Trajectory pitToCrypto2 = robot.drive.trajectoryBuilder(new Pose2d(cryptoToPit2.end().pos(), -yMultiplier * Math.PI / 2))
        ...
        if (elapsedTime < 28) {
            ...
            robot.drive.followTrajectory(robot.drive.trajectoryBuilder(pitToCrypto2.end())
                    .forward(8)
                    .build());
            robot.drive.waitForTrajectoryFollower();
