...
import static com.acmerobotics.relicrecovery.vision.VisionConstants.*;
...
public class CryptoboxTracker implements Tracker {
    ...
    private static void smartHsvRange(Mat src, Scalar lowerHsv, Scalar upperHsv, Mat dest) {
    ...
    private static CryptoboxResult findCryptobox(Mat src, double timestamp) {
        List<MatOfPoint> contours = new ArrayList<>();
        Mat morphedMask = new Mat();
        Mat openKernel = Mat.ones(OPEN_KERNEL_SIZE, OPEN_KERNEL_SIZE, CvType.CV_8U);
        Mat closeKernel = Mat.ones(CLOSE_KERNEL_SIZE, CLOSE_KERNEL_SIZE, CvType.CV_8U);
        ...
        Imgproc.findContours(morphedMask, contours, new Mat(), Imgproc.RETR_TREE, Imgproc.CHAIN_APPROX_SIMPLE);
        ...
        while (i < contours.size()) {
            ...
            line.get(0, 0, params);
            Log.i("CryptoboxTracker", line.dump());
        ...
        if (lineXs.size() >= 4) {
            ...
            for (int j = 1; j < lineXs.size(); j++) {
                if (lineXs.get(j) - lineXs.get(j - 1) > 0.25) {
    ...
    public synchronized void processFrame(Mat frame, double timestamp) {
        Mat resized = new Mat(), hsv = new Mat(), red = new Mat(), blue = new Mat();
