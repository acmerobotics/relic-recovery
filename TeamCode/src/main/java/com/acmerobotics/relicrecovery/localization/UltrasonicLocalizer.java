        ...
        if (useUltrasonicFeedback) {
            ...
            if (ultrasonicDistance > drive.getMinUltrasonicDistance(DistanceUnit.INCH)) {
                switch (closestCryptobox) {
                    case NEAR_BLUE:
                        estimatedPosition = new Vector2d(estimatedPosition.x(), -72 + AutoPaths.CRYPTO_COL_DEPTH + ultrasonicDistance);
                        break;
                    case NEAR_RED:
                        estimatedPosition = new Vector2d(estimatedPosition.x(), 72 - AutoPaths.CRYPTO_COL_DEPTH - ultrasonicDistance);
                        break;
                    case FAR_BLUE:
                    case FAR_RED:
                        estimatedPosition = new Vector2d(-72 + AutoPaths.CRYPTO_COL_DEPTH + ultrasonicDistance, estimatedPosition.y());
                        break;
                }
            }
        }
